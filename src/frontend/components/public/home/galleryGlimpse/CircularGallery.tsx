...
const DEFAULT_FONT = 'italic 24px Lora';
const DEFAULT_FONT_URL = 'https://fonts.googleapis.com/css2?family=Lora:ital,wght@1,500;1,600&display=swap';
...
async function resolveFont(font: string, fontUrl?: string): Promise<string> {
  ...
  if (!effectiveUrl) {
    if (document.fonts && document.fonts.load) {
      try {
        ...
        // Fall back to available browser fonts
      }
    }
    return font;
  }
  try {
    ...
    const prefix = sizeMatch ? sizeMatch[1].trim() : 'italic 24px';
...
function createTextTexture(
  gl: GL,
  text: string,
  font: string = 'italic 24px Lora, serif',
  color: string = '#1c1917'
  ...
  const paddingX = 40;
  const paddingY = 24;
  const rawW = textWidth + paddingX;
  const rawH = textHeight + paddingY;

  canvas.width = Math.ceil(rawW * dpr);
  canvas.height = Math.ceil(rawH * dpr);
  ...
  context.clearRect(0, 0, rawW, rawH);
  context.fillText(text, rawW / 2, rawH / 2);

  const texture = new Texture(gl, { generateMipmaps: false });
  texture.image = canvas;
  return { texture, width: rawW, height: rawH };
}
...
class Title {
  ...
  constructor({ gl, plane, renderer, text, textColor = '#1c1917', font = 'italic 24px Lora, serif' }: TitleProps) {
    autoBind(this);
    this.gl = gl;
    this.plane = plane;
...
class App {
  ...
  addEventListeners() {
    ...
    // Attach wheel and touch down directly to container so it doesn't hijack whole page scrolling
    this.container.addEventListener('wheel', this.boundOnWheel, { passive: true });
    this.container.addEventListener('mousedown', this.boundOnTouchDown);
    this.container.addEventListener('touchstart', this.boundOnTouchDown, { passive: true });

    // Track move and up on window so drag gestures don't get interrupted
...
export default function CircularGallery({
  ...
  return (
    <div
      ...
      tabIndex={0}
      role="region"
      aria-label="Interactive circular image gallery. Drag or use Left and Right Arrow keys to navigate."
    />
  );
}
